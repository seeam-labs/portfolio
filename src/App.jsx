...
export default function App() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['home', 'about', 'services', 'why', 'experience', 'awards', 'projects', 'contact'];
  ...
  return (
    <div className="bg-slate-950 text-slate-200 min-h-screen font-sans selection:bg-cyan-500/30 selection:text-cyan-200 relative overflow-hidden transition-colors duration-500">
        ...
        <section id="home" className="min-h-[85vh] flex flex-col-reverse md:flex-row items-center justify-between gap-12">
          <div className="flex flex-col justify-center items-start md:w-1/2">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <a href="CV_SEEAM_RAHMAN.pdf" target="_blank" rel="noreferrer" className="px-8 py-4 rounded-xl bg-slate-800/80 text-white font-bold border border-slate-700 hover:bg-slate-800 hover:border-cyan-500/50 transition-all flex items-center gap-2 backdrop-blur-sm group">
                {t.hero.btnCV} <FileText size={20} className="group-hover:text-cyan-400 transition-colors" />
              </a>
            </div>
          </div>
      ...
      <footer className="py-8 text-center text-slate-500 text-sm relative z-10 bg-slate-950/50 pb-36">
        <p>{t.contact.footer} <span className="text-slate-300 font-medium">Md. Seeam Rahman</span></p>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900/60 backdrop-blur-xl border border-slate-700/50 z-50 px-6 py-3.5 flex justify-between items-center shadow-[0_10px_40px_-10px_rgba(0,0,0,0.7)] rounded-full w-[90%] max-w-md transition-all duration-500 hover:bg-slate-900/80">
        {navLinks.map((link) => (
          <a
            ...
            href={`#${link.id}`}
            title={link.name}
            className={`relative p-2.5 rounded-full transition-all duration-300 group ${activeSection === link.id ? 'text-cyan-400 bg-cyan-500/10' : 'text-slate-400 hover:text-cyan-300 hover:bg-white/5'}`}
          >
            <link.icon size={22} className={`${activeSection === link.id ? 'scale-110 drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]' : 'group-hover:scale-110'} transition-transform`} />
            <span className="absolute -top-12 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-xs px-3 py-1.5 rounded-md opacity-0 group-hover:opacity-100 transition-all duration-300 md:block hidden pointer-events-none whitespace-nowrap shadow-lg border border-slate-700">
              {link.name}
